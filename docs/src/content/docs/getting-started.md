...
The `NEW-VERSION` argument can be specified in several ways:

1. **Omitted**: Uses the `versioning.policy` from `.craft.yml` (requires `minVersion >= 2.14.0`)
...
When no git tags exist (first release), Craft defaults to version `0.1.0` when using auto-versioning. This ensures a sensible starting point for new projects.
...
Positionals:
  NEW-VERSION  The new version to release. Can be: a semver string (e.g.,
               ...
               for calendar versioning. If omitted, uses versioning.policy.
                                                                        [string]
