...
describe("isPreviewRelease", () => {
  test.each(["preview", "pre", "alpha.0", "beta", "rc.1", "dev"])("accepts semver preview release", previewSuffix => {
...
describe("versionGreaterOrEqualThan", () => {
  function semVerFactory(major: number, minor: number, patch: number, pre?: string, build?: string): SemVer {
...
describe("semVerToString", () => {
  test.each([
    ...
    ["with pre-release", { major: 1, minor: 2, patch: 3, pre: "beta.1" }, "1.2.3-beta.1"],
    ["with build metadata", { major: 1, minor: 2, patch: 3, build: "linux" }, "1.2.3+linux"],
    [
      "with pre-release and build metadata",
      { major: 1, minor: 2, patch: 3, pre: "beta.1", build: "linux" },
      "1.2.3-beta.1+linux",
    ],
  ])("converts a SemVer object (%s) to a string", (_, semver, expectedString) => {
